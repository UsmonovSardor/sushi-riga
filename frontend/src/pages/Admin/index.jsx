...
const Stars = ({ n, size = 16, onClick }) => (
  <span style={{ display: 'inline-flex', gap: 2 }}>
    {[1, 2, 3, 4, 5].map(i => (
      <span key={i} onClick={onClick ? () => onClick(i) : undefined} style={{ fontSize: size, color: i <= n ? '#f59e0b' : '#d1d5db', cursor: onClick ? 'pointer' : 'default', lineHeight: 1 }}>★</span>
    ))}
  </span>
);
    ...
    <div style={{ minHeight: '100vh', background: '#f8fafc', fontFamily: 'Inter,system-ui,sans-serif' }}>
      ...
      <div style={{ background: '#fff', borderBottom: '1px solid #e2e8f0', padding: '0 24px', display: 'flex', gap: 2, overflowX: 'auto', position: 'sticky', top: 64, zIndex: 90 }}>
        {[
          ...
          <button key={k} onClick={() => { setTab(k); setSrch(''); if (k !== 'add') { setEdit(null); setForm(emptyForm()); } }} style={{ padding: '14px 18px', fontWeight: 800, fontSize: '.82rem', background: tab === k ? '#fff5f5' : 'transparent', border: 'none', borderBottom: `3px solid ${tab === k ? '#e31e24' : 'transparent'}`, color: tab === k ? '#e31e24' : '#64748b', cursor: 'pointer', whiteSpace: 'nowrap', fontFamily: 'Inter,sans-serif' }}>
            {label}
          </button>
        ))}
      </div>
          ...
          <div>
            <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
              ...
              <select value={stF} onChange={e => setStF(e.target.value)} style={{ ...S.inp, width: 200, cursor: 'pointer' }}>
                ...
                {Object.entries(STATUS).map(([k, v]) => <option key={k} value={k}>{v.icon} {v.label} ({orders.filter(o => o.status === k).length})</option>)}
              </select>
              ...
              {filtMenu.map(item => (
                <div key={item.id} style={{ ...S.card, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                  <div style={{ position: 'relative', paddingTop: '65%', background: '#f8fafc', overflow: 'hidden' }}>
                    {item.img ? <img src={item.img} alt="" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover' }} onError={e => e.target.style.display = 'none'} /> : <span style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '2.8rem' }}>{item.e}</span>}
          ...
          <div>
            <div style={{ ...S.card, padding: '20px', marginBottom: 20, display: 'flex', gap: 20, flexWrap: 'wrap', alignItems: 'center' }}>
              {[1, 2, 3, 4, 5].map(star => {
                ...
                return <div key={star} style={{ display: 'flex', alignItems: 'center', gap: 6 }}><Stars n={star} size={18} /><span style={{ fontSize: '.85rem', fontWeight: 700 }}>{cnt}</span></div>;
              })}
            ...
            <form onSubmit={saveItem}>
              ...
              <div style={{ marginBottom: 20 }}>
                ...
                {imgMode === 'url' ? (
                  ...
                  <div style={{ border: '2px dashed #e2e8f0', borderRadius: 12, padding: '24px', textAlign: 'center', background: '#f8fafc', cursor: 'pointer' }} onClick={() => fileRef.current?.click()}>
                    <input ref={fileRef} type="file" accept="image/*" style={{ display: 'none' }} onChange={e => handleFile(e.target.files[0])} />
                    {imgLoad ? <div style={{ color: '#64748b' }}>⏳ Yuklanmoqda...</div> : <div><div style={{ fontSize: '2.5rem', marginBottom: 8 }}>📸</div><div style={{ fontSize: '.84rem', color: '#64748b' }}>Rasm tanlang yoki bu yerga tashlang</div></div>}
                  </div>
                )}
              </div>
              ...
              <div style={{ display: 'flex', gap: 12 }}>
                ...
                <button type="button" style={{ ...S.btn('#f1f5f9', '#475569'), height: 46, padding: '0 24px' }} onClick={() => { setTab('menu'); setEdit(null); setForm(emptyForm()); }}>Bekor</button>
              </div>
            </form>
          </div>
        )}
