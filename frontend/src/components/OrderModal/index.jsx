...
function StripeForm({ amount, onSuccess, onBack, lang }) {
  ...
  return (
    <div className="stripe-wrap">
      ...
      <div className="pay-hint">Visa · Mastercard · Apple Pay · Google Pay</div>
      <div className="stripe-elem-wrap"><PaymentElement options={{ layout: 'tabs' }} /></div>
      {err && <div className="order-error">{err}</div>}
      <button className="btn-primary" onClick={pay} disabled={busy} style={{marginTop:14}}>
        {busy ? '⏳' : `💳 ${lang==='lv'?'Apmaksāt':lang==='en'?'Pay':'Оплатить'} €${amount}`}
      </button>
      <button className="btn-secondary" onClick={onBack}>
        ← {lang==='lv'?'Atpakaļ':lang==='en'?'Back':'Назад'}
      </button>
    </div>
  );
}
      ...
      <div className="order-sheet">
        ...
        {step === 'form' && (
          <>
            ...
            <div className="order-total">
              <span>{lbl('Kopā','Итого','Total')}</span>
            ...
            <div className="form-label" style={{marginBottom:8}}>
              {lbl('Maksājuma veids','Способ оплаты','Payment')}
            </div>
            <div className="pay-methods">
              {[
                ['cash', '💵', lbl('Nauda pie saņemšanas','Наличными при получении','Cash on pickup')],
                ['card', '💳', 'Visa / Mastercard / Apple Pay'],
              ].map(([k, icon, label]) => (
                <button key={k} type="button"
                  className={'pay-method' + (pay === k ? ' on' : '')}
                  onClick={() => setPay(k)}>
            ...
            <button className="btn-primary" onClick={submit} disabled={loading} style={{marginTop:12}}>
              {loading ? '⏳' : pay === 'cash'
                ? lbl('✅ Apstiprināt pasūtījumu','✅ Подтвердить заказ','✅ Confirm order')
                : `💳 ${lbl('Doties uz apmaksu →','Перейти к оплате →','Pay now →')}`}
            </button>
        ...
        {step === 'done' && (
          <div className="order-success">
            ...
            <div className="order-success-t">{lbl('Pasūtījums pieņemts!','Заказ принят!','Order received!')}</div>
            <div className="order-success-num">#{orderId}</div>
            <div className="order-success-p">
              {lbl(
                'Paldies! Mēs sazināsimies ar jums drīzumā.',
