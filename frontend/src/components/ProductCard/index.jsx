    ...
    <div className="card" style={{animationDelay:delay+'ms'}} onClick={() => add(item)}>
      ...
      <div className="card-body">
        ...
        <div className="card-foot">
          <div><span className="card-price">€{item.price.toFixed(2)}</span>{item.old&&<span className="card-old">€{item.old.toFixed(2)}</span>}</div>
          <button className="card-add" onClick={e=>{e.stopPropagation();add(item);}}>+</button>
        </div>
      </div>
    </div>
  );
