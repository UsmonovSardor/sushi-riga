...
export default function ProductCard({ item, delay = 0, reviewSummary = null }) {
      ...
      <div className="card-body">
        ...
        {count > 0 && (
          <div style={{ display:'flex', alignItems:'center', gap:4, marginBottom:4 }}>
            <span style={{ color:'#f59e0b', fontSize:'.82rem', letterSpacing:'-.5px' }}>
              {'★'.repeat(Math.round(avg))}{'☆'.repeat(5 - Math.round(avg))}
            </span>
            <span style={{ fontSize:'.72rem', color:'#94a3b8', fontWeight:600 }}>{avg} ({count})</span>
          </div>
        )}
