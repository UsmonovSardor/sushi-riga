...
exports.getMenu = [authAdmin, (_req, res) => {
  res.json(load(MENU_FILE));
}];
...
exports.getOrders = [authAdmin, (_req, res) => {
...
exports.updateOrder = [authAdmin, (req, res) => {
  ...
  if (nextStatus === 'ready' && prevStatus !== 'ready') {
    orders[idx].readyAt = orders[idx].updatedAt;
  }

  if (nextStatus === 'delivered' && prevStatus !== 'delivered') {
    orders[idx].deliveredAt = orders[idx].updatedAt;
  }
